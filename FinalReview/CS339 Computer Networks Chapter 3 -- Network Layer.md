...
When sender receives duplicated packet with same `ACK` number, if the sender is indeed doing retransmission, it will go to transmit the next packet. If the sender is sending another packet, it knows this packet fails to reach receiver correctly, and it will resent it.
